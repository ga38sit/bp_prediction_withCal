...
class BPHead(tf.keras.layers.Layer):
    def __init__(self, name_prefix: str, name: str = None, **kwargs):
        ...
        self.conv64 = tf.keras.layers.Conv1D(16, kernel_size=3, padding="same", name=f"{self.name_prefix}_conv64")
...
def build_deep_bp_og_small(
    ...
    print(f"Building Deep-BP OG small model with input shape: {input_shape}")
    ...
    x = layers.Conv1D(filters=32, kernel_size=3, padding="same", name="stem_conv2")(x)
    ...
    x = ResidualBlock(filters=32, stride=1, name="res1")(x)
    x = ResidualBlock(filters=64, stride=2, name="res2")(x)
    x = ResidualBlock(filters=128, stride=2, name="res3")(x)
    # x = ResidualBlock(filters=512, stride=2, name="res4")(x)

    inputs = [signal_in]
    model_name = "DeepBP_small"
